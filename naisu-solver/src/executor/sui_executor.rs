...
async fn get_initial_shared_version(sui: &SuiClient, object_id: ObjectID) -> eyre::Result<SequenceNumber> {
    ...
    if let Some(owner) = data.owner {
        if let sui_types::object::Owner::Shared { initial_shared_version } = owner {
...
/// Legacy v1 (no Wormhole) — kept for backward compatibility.
pub async fn fulfill(config: &Config, order: &EvmOrder, amount_to_send: u64) -> Result<String> {
    let (keypair, sender) = build_client_and_key(config)?;
    ...
    for attempt in 0..total_attempts {
        ...
        if attempt > 0 {
            let delay = if attempt % rpc_urls.len() == 0 { RETRY_DELAY_SECS * 2 } else { RETRY_DELAY_SECS };
            info!(rpc = %rpc_url, attempt = attempt + 1, round, "Trying Sui RPC for fulfill...");
            tokio::time::sleep(tokio::time::Duration::from_secs(delay)).await;
        }
        ...
        let gas_coin = match coins.data.first() {
            Some(c) => c,
            None => return Err(eyre::eyre!("No coins")),
        };

        let tx_data = match sui
            .transaction_builder()
            .transfer_sui(sender, gas_coin.coin_object_id, 10_000_000, recipient, Some(amount_to_send))
            .await
        {
            Ok(td) => td,
            Err(e) => {
                let err_str = format!("{e}");
                warn!(rpc = %rpc_url, "transfer_sui failed: {err_str}");
                last_err = err_str.clone();
                if is_retryable_error(&err_str) { continue; }
                return Err(eyre::eyre!("{e}"));
            }
            ...
            .await
        {
            Ok(response) => {
                let digest = response.digest.to_string();
                info!(digest = %digest, to = %recipient, amount = amount_to_send, "SUI sent (legacy)!");
                return Ok(digest);
            }
            Err(e) => {
                let err_str = format!("{e}");
                warn!(rpc = %rpc_url, "execute_transaction_block(fulfill) failed: {err_str}");
                last_err = err_str.clone();
                if !is_retryable_error(&err_str) {
                    return Err(eyre::eyre!("execute_transaction_block failed: {err_str}"));
                }
    ...
    Err(eyre::eyre!("fulfill failed after {} attempts ({} rounds). Last error: {}", total_attempts, MAX_ROUNDS, last_err))
}

/// Legacy v1 (no Wormhole) — kept for reference.
pub async fn claim_intent(config: &Config, intent_object_id: &str) -> Result<String> {
    let (keypair, sender) = build_client_and_key(config)?;
    let package_id: ObjectID = config.sui_package_id.parse()?;
    ...
    for attempt in 0..total_attempts {
        ...
        if attempt > 0 {
            let delay = if attempt % rpc_urls.len() == 0 { RETRY_DELAY_SECS * 2 } else { RETRY_DELAY_SECS };
            info!(rpc = %rpc_url, attempt = attempt + 1, round, "Trying Sui RPC for claim_intent...");
            tokio::time::sleep(tokio::time::Duration::from_secs(delay)).await;
        }
        ...
        let tx_data = match sui
            ...
            .move_call(
                ...
                vec![],
                vec![
                    SuiJsonValue::new(serde_json::json!(intent_object_id))
                        .map_err(|e| eyre::eyre!("{e}"))?,
                    SuiJsonValue::new(serde_json::json!("0x6")).map_err(|e| eyre::eyre!("{e}"))?,
                ],
                None,
                10_000_000,
                None,
            ...
            .await
        {
            ...
            Err(e) => {
                let err_str = format!("{e}");
                warn!(rpc = %rpc_url, "move_call(fulfill_intent) failed: {err_str}");
                last_err = err_str.clone();
                if is_retryable_error(&err_str) { continue; }
                return Err(eyre::eyre!("{e}"));
            }
            ...
            .await
        {
            Ok(response) => {
                let digest = response.digest.to_string();
                info!(digest = %digest, rpc = %rpc_url, "fulfill_intent called (legacy)!");
                return Ok(digest);
            }
            Err(e) => {
                let err_str = format!("{e}");
                warn!(rpc = %rpc_url, "execute_transaction_block(claim_intent) failed: {err_str}");
                last_err = err_str.clone();
                if !is_retryable_error(&err_str) {
                    return Err(eyre::eyre!("execute_transaction_block failed: {err_str}"));
                }
    ...
    Err(eyre::eyre!("claim_intent failed after {} attempts ({} rounds). Last error: {}", total_attempts, MAX_ROUNDS, last_err))
}
