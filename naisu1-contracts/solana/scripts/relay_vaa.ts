...
interface ParsedVaa {
  ...
  body: Buffer;
  bodyHash: Buffer;   // keccak256(body) — used as PostedVAA seed
  doubleHash: Buffer; // keccak256(bodyHash) — what guardians sign
  timestamp: number;
  nonce: number;
  emitterChain: number;
...
function parseVaa(hex: string): ParsedVaa {
  const buf = Buffer.from(hex, 'hex');
  let offset = 0;

  const version = buf[offset++];
  const guardianSetIndex = buf.readUInt32BE(offset); offset += 4;
  const sigCount = buf[offset++];

  const signatures: VaaSignature[] = [];
  for (let i = 0; i < sigCount; i++) {
    const guardianIndex = buf[offset++];
    const r = buf.slice(offset, offset + 32); offset += 32;
    const s = buf.slice(offset, offset + 32); offset += 32;
    const recoveryId = buf[offset++];
    signatures.push({ guardianIndex, r, s, recoveryId });
  }

  const body = buf.slice(offset);
  const bodyHash   = Buffer.from(keccak_256(body));
  const doubleHash = Buffer.from(keccak_256(bodyHash));
...
function parseGuardianSet(data: Buffer): { index: number; keys: Buffer[] } {
  // Skip 8-byte Anchor discriminator
  let offset = 8;
  const index   = data.readUInt32LE(offset); offset += 4;
  const keysLen = data.readUInt32LE(offset); offset += 4;
  const keys: Buffer[] = [];
 ...
 */
function buildVerifySignaturesIx(
  wormholeProgram: PublicKey,
  payer: PublicKey,
  guardianSetPda: PublicKey,
  signatureSet: PublicKey,
  signersArray: number[], // 19 i8 values; -1 = not included, 0+ = secp256k1 instruction index
): TransactionInstruction {
 ...
 */
function buildPostVaaIx(
  wormholeProgram: PublicKey,
  payer: PublicKey,
  guardianSetPda: PublicKey,
  bridgePda: PublicKey,
  signatureSet: PublicKey,
  postedVaaPda: PublicKey,
  vaa: ParsedVaa,
): TransactionInstruction {
  ...
  let off = 0;
  disc.copy(body, off); off += 8;
  body[off++] = vaa.version;
  body.writeUInt32LE(vaa.guardianSetIndex, off); off += 4;
  body.writeUInt32LE(vaa.timestamp, off); off += 4;
  body.writeUInt32LE(vaa.nonce, off); off += 4;
  body.writeUInt16LE(vaa.emitterChain, off); off += 2;
  vaa.emitterAddress.copy(body, off); off += 32;
  body.writeBigUInt64LE(vaa.sequence, off); off += 8;
  body[off++] = vaa.consistencyLevel;
  body.writeUInt32LE(payload.length, off); off += 4;
...
async function main() {
  ...
  const wormholeProgram = new PublicKey(wormholeProgramIdStr);
  ...
  const [guardianSetPda] = PublicKey.findProgramAddressSync(
    [Buffer.from('GuardianSet'), guardianIndexBuf],
    wormholeProgram
  );
  ...
  const [postedVaaPda] = PublicKey.findProgramAddressSync(
    [Buffer.from('PostedVAA'), vaa.bodyHash],
    wormholeProgram
  );
  ...
  if (!gsAccount) throw new Error(`GuardianSet not found: ${guardianSetPda}`);
  ...
  const secp256k1Ixs: TransactionInstruction[] = [];
  const signersArr: number[] = new Array(19).fill(-1);

  for (let i = 0; i < vaa.signatures.length; i++) {
    const sig = vaa.signatures[i];
    const ethAddress = guardianSet.keys[sig.guardianIndex];
    if (!ethAddress) throw new Error(`Guardian index ${sig.guardianIndex} not in set`);
  ...
  const verifyIx = buildVerifySignaturesIx(
    wormholeProgram,
    payer.publicKey,
    guardianSetPda,
  ...
  for (const ix of secp256k1Ixs) verifyTx.add(ix);
  verifyTx.add(verifyIx);

  console.error('Submitting verify_signatures...');
  const verifySig = await sendAndConfirmTransaction(
    connection, verifyTx, [payer, signatureSetKeypair], { commitment: 'confirmed' }
  );
  console.error(`verify_signatures confirmed: ${verifySig}`);

  // ── Build and submit post_vaa ─────────────────────────────────────────────
  const postVaaIx = buildPostVaaIx(
    wormholeProgram, payer.publicKey,
    guardianSetPda, bridgePda,
    signatureSetKeypair.publicKey, postedVaaPda, vaa,
  );
  const postVaaTx = new Transaction();
  postVaaTx.add(postVaaIx);

  console.error('Submitting post_vaa...');
  const postVaaSig = await sendAndConfirmTransaction(
    connection, postVaaTx, [payer], { commitment: 'confirmed' }
  );
