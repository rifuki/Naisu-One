...
async function main() {
    ...
    // ── Derive PDAs ──────────────────────────────────────────────────────────────
    const [pool, poolBump] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from('pool'), authority.publicKey.toBytes()], liquidStakingProgram);
    const [lstMint, lstMintBump] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from('lst_mint'), pool.toBytes()], liquidStakingProgram);
    const [wsolVault, vaultBump] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from('wsol_vault'), pool.toBytes()], liquidStakingProgram);
    ...
    const sig = await (0, web3_js_1.sendAndConfirmTransaction)(connection, tx, [authority], { commitment: 'confirmed' });
