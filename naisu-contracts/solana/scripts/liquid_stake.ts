...
function buildSyncNativeIx(wsolAta: PublicKey): TransactionInstruction {
...
function buildStakeIx(
  liquidStakingProgram: PublicKey,
  user: PublicKey,
  pool: PublicKey,
  lstMint: PublicKey,
  wsolVault: PublicKey,
...
async function main() {
  ...
  const poolAuthority = new PublicKey(poolAuthorityB58);
  ...
  const [pool] = PublicKey.findProgramAddressSync(
    [Buffer.from('pool'), poolAuthority.toBytes()],
