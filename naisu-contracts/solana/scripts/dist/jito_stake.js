...
function loadMintAddress() {
    ...
    if (process.env.MOCK_JITO_MINT)
        return process.env.MOCK_JITO_MINT;
    ...
    if (!config.jitoSOL)
        throw new Error('jitoSOL mint not found in mock_tokens.json');
...
async function main() {
    ...
    const recipientAta = await (0, spl_token_1.getOrCreateAssociatedTokenAccount)(connection, solver, // payer (creates ATA if needed)
    mint, recipient);
    ...
    const txSig = await (0, spl_token_1.mintTo)(connection, solver, // payer
    mint, // mint
